    ...
    <div class="container-scroller">
      ...
      @include('admin.navbar')
        ...
        <form method="POST" action="{{ url('addaccesstouser') }}" autocomplete="off">
            @csrf
            ...
            <select class="col-md-4 form-control" id="code" name="code"  >
                @foreach ($access as $item )
                <option value="{{ $item->code }}">{{ $item->access.' [code: '.$item->code.']' }}</option> 
                @endforeach
